...
initial_wealth = [(max(min(random.gauss(mean, 0.1), max_value), min_value)) for _ in range(num_entries)]

min_wage_factor = 0.25
...
#df.to_csv('symmetric_correlation_distribution.csv', index=False)
...
# Save combined plot
# plt.savefig('symmetric_correlation_distribution.png', bbox_inches='tight',pad_inches = 0.3,dpi = 200)
plt.show()