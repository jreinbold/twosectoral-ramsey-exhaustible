...
initial_wealth = [(max(min(random.gauss(mean, sigma), max_value), min_value)) for _ in range(num_entries)]


productivity = []
for initial_wealth_item in initial_wealth:
    productivity_item = max(min(random.gauss(initial_wealth_item, 100), 3000), 0)
    productivity.append(productivity_item/500)
    
initial_wealth = [x / 500 for x in initial_wealth]
...
#df.to_csv('correlation_distribution.csv', index=False)

print(sum(productivity) / len(productivity))
print(sum(initial_wealth) / len(initial_wealth))


# =============================================================================
# Single plots distributions
# =============================================================================
...
# Save combined plot
# plt.savefig('distribution_symmetric_correlation.png', bbox_inches='tight',pad_inches = 0.3,dpi = 200)
plt.show()